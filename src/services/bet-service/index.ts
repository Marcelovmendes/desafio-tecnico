...
async function postBet({
  ...
  const { amountInCents } = await validateBet({
    homeTeamScore,
    awayTeamScore,
    amountBet,
    gameId,
...
export async function validateBet(bet: checkBetParams) {
  if (!bet) throw missingFiledsError("Missing fields in bet");

  const balanceParticipant = await participantsRepository.findParticipantById(
    bet.participantId
  );
  if (
    !bet.amountBet ||
    bet.amountBet < 0 ||
    bet.amountBet > balanceParticipant.balance
  )
  ...
  const partcipant = await participantsRepository.findParticipantById(
    bet.participantId
  );
  if (partcipant.balance < amountInCents)
